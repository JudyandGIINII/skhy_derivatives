...
- **H1 종가 리밸런싱**: 무료 KRX 일별정보는 과거 확인·크로스체크·백필에만 쓰고, `NAV/IV × 상장좌수`와 일별 수익률을 쓰는 `h1_krx_daily_proxy_reduced_v1`로 분리한다. 이 경로는 `daily-proxy`·승격 비대상이며 원래 H1과 성과를 합치지 않는다. 원래 H1은 15:10 KST snapshot, 15:19:30 주문 의도 마감과 당시까지의 실시간 입력을 사용하며, 무료 KIS/Toss feed 통합은 후속 작업으로 남긴다. `kappa`는 학습 구간·상품·regime별 버전으로 관리하고 예상체결/프로그램 피드가 없으면 결측 상태를 보존한다. [FR-09]
...
| P1-02 | 무료 KRX 일별 국내 H1 universe의 NAV/IV·상장좌수·이름 기반 배율을 수집하고 listed-notional proxy·basis date·수신시각·lineage를 보존 | P1-01, G-04 | 동적 발견, proxy 단위, 직전 기준일/as-of, lineage 결측 제외 | FR-03, FR-04, FR-05, FR-09, FR-16 |
| P1-03 | H1 theoretical exposure, flow adjustment, ADV 정규화와 `h1_krx_daily_proxy_reduced_v1` 구현 | P1-02, G-03 | beta 2/-1/-2 부호, 상품·regime별 kappa, model/resolution/scope 분리, 원 H1 승격 차단 | FR-09, FR-16 |
...
| 3 | G-04 무료 KRX 일별 H1 universe·proxy·가용성·lineage | 선택된 국내 상품의 instrument ID, 이름 기반 beta, NAV/IV×상장좌수 proxy, basis date·수신시각·원천 record ID를 검증 | 불명확 상품을 일별 축소모델에서 제외 | Phase 1 백필·feature 전 |
| 4 | G-03 종가 예상체결·프로그램·호가 깊이 | 무료 공급자 필드 문서, 실제 sample, 시각·깊이·라이선스 확인 | 0으로 대체하지 않고 `missing` flag를 가진 H1 축소모델을 별도 version으로 평가 | H1 완전/축소모델 선택 전 |

...
- 이 문서는 G-04 최소조건의 재정의안을 제시할 뿐이다. `docs/decisions/gates/G-04.md`의
  `IN_REVIEW` 상태를 변경하지 않는다.
...
또한 현재 `src/`에는 `theoretical_delta_exposure`, `estimated_close_pressure`,
`KappaRegistry`를 KRX row 또는 `FundSnapshot`과 연결하는 production feature builder
call site가 없다. `tests/e2e/test_h1_pipeline_end_to_end.py`도 합성
`ClosePressureResult`를 직접 주입한다. 따라서 현 상태는 계산·전략 계약이 구현된 것이며,
무료 KRX 원천 필드가 H1 입력으로 완전히 배선됐다고 볼 수는 없다.
...
## 5. 무료 API 전용 G-04 최소 요구조건 재정의 제안

### 5.1 게이트 질문을 좁히는 안

현재 G-04의 질문은 국내 단일종목 레버리지 상품 전체에 대해 PCF·AUM/NAV 공개시각과
복제방식을 확정하는 것이다. 무료 KRX만 사용한다는 사용자 방침과 실제 H1 코드의 직접
소비 범위를 반영하면, 축소모델용 질문은 다음처럼 좁힐 수 있다.

> **제안 G-04 질문:** 무료 KRX ETF/ETN 일별정보만으로 H1에 포함할 국내 SK하이닉스
> 단일종목 레버리지 상품을 거래일별로 재현하고, 배율과 직전 listed-notional proxy를
> 산출하며, 그 값이 의사결정 시점에 사용 가능했음을 허위 timestamp 없이 판정할 수
> 있는가?

이 제안은 G-04 상태 변경이 아니라 오케스트레이터가 검토할 acceptance scope다.

### 5.2 제안 acceptance criteria
...
단, 정확한 게시시각을 비필수로 돌리는 대신 위 acceptance criterion 4의 실제 수신시각과
보수적 가용성 규칙을 반드시 둬야 한다. 현재 `FundSnapshot`/lookahead guard는
`published_at`을 요구하므로 이 재정의를 채택하더라도 production 배선이 자동으로
완성되는 것은 아니다.

### 5.4 G-04가 좁아져도 남는 경계
...
축소 gate로 좁힐 수 있다. 이 경우에도 모델 이름과 해상도를 분리하고, 원래 15:10 H1과
성과를 합치지 않는 것이 최소 안전조건이다.
